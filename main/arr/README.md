...
26. We also want the **LXC** to boot properly on startup, go to **Options** tab and set **Start/Shutdown order** to `6`. 

27. We also want to give the **LXC** access to our `media` and `downloads` datasets from [truenas](./../truenas/README.md), so open the **Proxmox Node**'s **Shell**.
...
6. Scroll down to **Anna's Archive** and enable **Anna's Archive Enabled**. // TODO: Custom Acquire Proxy
...
15. Set **Active Library Platform** to **BookOrbit**.

16. Set the **BookOrbit URL** to `http://172.20.101.10:3131`.

17. Set the **BookOrbit** **Username** and **Password** to the admin user you created during [BookOrbit Configuration](./../media/README.md#BookOrbit).

18. Scroll down and **Test Library Platform Connection**.

19. Scroll back up and for **Audiobook Library (BookOrbit)** set it to **Audiobooks (bookorbit)**.

20. For **Ebook Library (BookOrbit)** set it to **Ebooks (bookorbit)**.

21. Scroll up and **Save All** and then scroll back down and **Sync BookOrbit Inventory**.

22. (optional) Scroll down to **Hardcover**. If you already created a **Hardcover** account during [BookOrbit Configuration](./../media/README.md#BookOrbit), you can skip the next step.

23. (optional) Create a **Hardcover** account ([from here](https://hardcover.app/)).

24. (optional) In **Hardcover** under **Settings** go to **Hardcover API**.

25. (optional) Click **New API Key** and **Name** it "Shelfarr" and set the **Expiration** to **Never**. Set **Permissions** to **All** and **Create Key**. Now **Copy** the key.

26. (optional) Paste the API key into the **Hardcover API Token** field back in the **Shelfarr** WebUI, hit **Test Hardcover Connection** and enable **Hardcover Enalbed** and scroll up and **Save All**!

27. Now go to the **Queue & System** settings tab.

28. Under **Queue Settings** enable:
    - **Immediate Search Enabled**
    - **Auto Approve Requests**

    And scroll up and **Save All**.

29. Now click **Admin** in the top bar and click **Download Clients**.

30. Click **New Client**.

31. Set the **Type** to **Qbittorrent** and set the **Name** to `QBitTorrent`.

32. Set the **URL** to `http://172.24.1.10:8080`.

33. Set the **Username** and **Password** to the admin credentials you created during the [QBitTorrent](#QBitTorrent) setup.

34. Set **Torrent Verification Attempts** to `50` and **Verification Wait Time** to `15` seconds.

35. Now **Create Download Client**. You should now be able to request your books! :)
